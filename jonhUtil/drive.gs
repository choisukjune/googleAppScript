...
/**
 * 자신의 드라이브 폴더기준 하위파일리스트를 반환한다.
 */
function getRootForderList( ){
    console.log( "[ S ] - getRootForderList" )
    var forders = DriveApp.getRootFolder().getFolders()
    var r = [];
    while(forders.hasNext()){
      var forder = forders.next();
      var o = {
        id : forder.getId(),
        nm : forder.getName(),
        cdt : forder.getDateCreated(),
        udt : forder.getLastUpdated(),
        ownerNm : forder.getOwner().getName(),
        ownerEmail : forder.getOwner().getEmail(),
        url : forder.getUrl(),
      }
      r.push( o )
    }
    consoleLogDateTime( JSON.stringify( r,null,4 ) )
    console.log( "[ E ] - getRootForderList" )
    return r
}
 ...
 * 자신의 드라이브 폴더기준 하위파일리스트를 반환한다.
 */
function getAllForderList(){
    console.log( "[ S ] - getAllForderList" )
    var forders = DriveApp.getFolders()
    var r = [];
    while(forders.hasNext()){
      var forder = forders.next();
      var o = {
    ...
    consoleLogDateTime( JSON.stringify( r ) )
    console.log( "[ E ] - getAllForderList" )
    return r
}

function getFolderByName( nm ){
  console.log( "[ S ] - getFolderByName" )
  nm = nm || "tmp"
  var forders = DriveApp.getRootFolder().getFoldersByName( nm );
  var r = [];
  while(forders.hasNext()){
    var forder = forders.next();
    var o = {
...
function getFolderById( id ){
  console.log( "[ S ] - getFolderByName" )
  nm = nm || "tmp"
  var forders = DriveApp.getRootFolder().getFolders( nm );
  var r = [];
  while(forders.hasNext()){
    var forder = forders.next();
    var o = {
...
/**
 * 드라이브 폴더기준 하위파일리스트를 반환한다.
 */
function getFileLsitByFolderIdToArr( folderId ){
    console.log( "[ S ] - getFileLsitByFolderIdToArr" )
    ...
    console.log( "[ E ] - getFileLsitByFolderIdToArr" )
    return r
}
